...
ALLOWED_DIRS = [PROJECT_ROOT / ".tmp", PROJECT_ROOT / "runs", PROJECT_ROOT / "output"]
...
def validate_write_path(path: str) -> Path:
    ...
    for allowed in ALLOWED_DIRS:
        ...
        if str(target).startswith(str(allowed)):
            return target
    raise PermissionError(f"Write blocked — {path} is outside allowed directories: {[str(d) for d in ALLOWED_DIRS]}")
